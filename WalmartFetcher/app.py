...
def update(sku):
     ...
     instock = str(product.available_online)
     ...
     for i, row in data_df.iterrows() :
        ...
        if data == lookup:
           ...
           st = str(st)
           print(st)
           final_price = (product.sale_price * 1.1765) + 3
           print(final_price)
...
#-----------------
           data_df['Min Price'] = data_df['Min Price'].replace([old_dataMP], '$'+str(final_price))
           data_df['In Stock'] = data_df['In Stock'].replace([old_dataIN], instock)
           data_df['Sold By'] = data_df['Sold By'].replace([old_dataSB], str(sold))
    ...
    if request.method == 'POST':
       try:
          ...
          if left_box:
             ...
             stock=left
             fields=[lookup + ',' + '$'+str(product.sale_price) + ',' + instock + ',' + stock + ',' + str(sold) + ',' + st + ',' + '$'+str(final_price)]
